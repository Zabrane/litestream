...
dist-linux:
	...
	docker run --rm -v "${PWD}":/usr/src/litestream -w /usr/src/litestream -e GOOS=linux -e GOARCH=amd64 golang:1.24.1 go build -v -ldflags "-s -w" -o dist/litestream ./cmd/litestream
